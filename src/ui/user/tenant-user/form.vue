    ...
    div
      el-form(ref="form", :model="formData", :rules="rules", label-width="300px")
        ...
        el-form-item(label="名字：", prop="name")
          el-input.input(placeholder="最多20个字", :maxlength="20", v-model="formData.name")
        el-form-item(label="手机号码：", prop="mobile")
          el-input.input(placeholder="手机号11位1开头", :maxlength="11", v-model="formData.mobile")
        el-form-item(label="密码：", prop="passwd")
          el-input.input(type="password", placeholder="请输入密码", v-model="formData.passwd")
        el-form-item(label="重复密码：", prop="checkpasswd")
          el-input.input(type="password", placeholder="请重复密码", v-model="formData.checkpasswd")
        div.border 公司信息
        el-form-item(label="公司名称：", prop="company_name")
          el-input.input(v-model="formData.company_name")
        el-form-item(label="公司地址：", prop="company_addr")
          el-input.input(v-model="formData.company_addr")
        el-form-item(label="公司官网：", prop="company_website")
          el-input.input(v-model="formData.company_website")
        el-form-item(label="公司描述：", prop="company_desc")
          el-input.input(type="textarea", :autosize="{ minRows: 3, maxRows: 5}", v-model="formData.company_desc")
        el-form-item(label="联系人：", prop="company_contact")
          el-input.input(v-model="formData.company_contact")
        el-form-item(label="手机号：", prop="company_mobile")
          el-input.input(v-model="formData.company_mobile")
        el-form-item(label="邮箱：", prop="company_email")
          el-input.input(v-model="formData.company_email")
        el-form-item
